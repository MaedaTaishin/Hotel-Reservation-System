...
public class ReservationSqlDao implements ReservationDao {
	...
	/**
	 * @see domain.reservation.ReservationDao#removeReservation(domain.reservation.Reservation)
	 */
	public void removeReservation(Reservation reservation) throws ReservationException {
		StringBuffer sql = new StringBuffer();
		Statement statement = null;
		ResultSet resultSet = null;
		Connection connection = null;
		try {
			...
			ReservationException exception = new ReservationException(
					ReservationException.CODE_DB_EXEC_QUERY_ERROR, e);
			exception.getDetailMessages().add("removeReservation()");
			throw exception;
		}
		finally {
			close(resultSet, statement, connection);
